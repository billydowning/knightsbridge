  ...
  try {
    ...
    useEffect(() => {
      if (connected && publicKey) {
        ...
        setBalance(0);
        setError(null);
        // 🚛 TOYOTA: Clear program cache when wallet disconnects
        programCacheRef.current = null;
      }
    }, [connected, publicKey]);

    // 🚛 TOYOTA: Simple program cache to prevent race conditions
    const programCacheRef = useRef<Program<ChessEscrow> | null>(null);
    ...
    const getProgram = async (): Promise<Program<ChessEscrow> | null> => {
      ...
      // 🚛 TOYOTA FIX: Return cached program if available and valid
      if (programCacheRef.current) {
